...
```{r}
brum_pop = final_df[(final_df$NOME_MUN == "Brumadinho" & final_df$ANO <= 2021), c("ANO", "POP")]
print(brum_pop)
```
...
```{r}
plot_df_abs = plot_df
plot_df_abs["Treated"] = plot_df_abs["Treated"] * brum_pop["POP"]
plot_df_abs["Synthetic"] = plot_df_abs["Synthetic"] * brum_pop["POP"]
...
ggplot() +
  # synthetic line and points (dashed)
  geom_line(data = plot_df_abs, 
            aes(x = Year, y = Synthetic/1e9, color = "Synthetic Control"), 
            size = 1.3, linetype = "dashed") +
  geom_point(data = plot_df_abs, 
             aes(x = Year, y = Synthetic/1e9, color = "Synthetic Control"), size = 2.5) +
  
  # treated line and points (solid)
  geom_line(data = plot_df_abs, 
            aes(x = Year, y = Treated/1e9, color = "Treated Unit"), 
            size = 1.3) +
  geom_point(data = plot_df_abs, 
             aes(x = Year, y = Treated/1e9, color = "Treated Unit"), size = 2.5) +
  
  # vertical line
  geom_vline(xintercept = 2019, linetype = "dotted", color = "black", size = 1) +
  
  scale_x_continuous(breaks = seq(min(plot_df_abs$Year), max(plot_df_abs$Year), 1)) +
  
  # y-axis breaks automatically scaled, with custom labels
  scale_y_continuous(
    breaks = seq(0, max(plot_df_abs$Treated/1e9, plot_df_abs$Synthetic/1e9) + 2, by = 1.5),
    labels = function(x) paste0("R$ ", x, " BI")
  ) +
  ...
  labs(
    x = "Year",
    y = "GDP Per Capita (Billions of R$)"
  ) +
  theme(
    text = element_text(size = 10),
    axis.text.x = element_text(angle = 45, hjust = 1),
  ...
  # annotation adjusted to the new scale
  annotate("text", 
           x = 2019, 
           y = max(plot_df_abs$Treated, plot_df_abs$Synthetic)/1e9,
...
```{r}
placebo_df_filtered_gpd = placebo_df_filtered
gap_data_gdp = gap_data
rmspe_stats_filtered_gdp = rmspe_stats_filtered
treated_ratio_gdp = treated_ratio
```


```{r}
placebo_df_filtered = placebo_df_filtered_gpd
gap_data = gap_data_gdp
rmspe_stats_filtered = rmspe_stats_filtered_gdp
treated_ratio = treated_ratio_gdp
...
sp <- ggplot() +
  ...
  geom_vline(xintercept = 2019, linetype = "dotted", color = "black", size = 1) +
  annotate("text", x = 2019, y = min(gap_data$Gap, placebo_df_filtered$Gap),
