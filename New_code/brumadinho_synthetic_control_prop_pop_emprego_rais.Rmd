...
#    list("REMUNERACAO_MEDIA", 2018, "mean")
 ...
 #   ,
    list("PROP_POP_EMPREGADA", 2002, "mean"),
    list("PROP_POP_EMPREGADA", 2003, "mean"),
    list("PROP_POP_EMPREGADA", 2004, "mean"),
    list("PROP_POP_EMPREGADA", 2005, "mean"),
    list("PROP_POP_EMPREGADA", 2006, "mean"),
    list("PROP_POP_EMPREGADA", 2007, "mean"),
    list("PROP_POP_EMPREGADA", 2008, "mean"),
    list("PROP_POP_EMPREGADA", 2009, "mean"),
    list("PROP_POP_EMPREGADA", 2010, "mean"),
    list("PROP_POP_EMPREGADA", 2011, "mean"),
    list("PROP_POP_EMPREGADA", 2012, "mean"),
    list("PROP_POP_EMPREGADA", 2013, "mean"),
    list("PROP_POP_EMPREGADA", 2014, "mean"),
    list("PROP_POP_EMPREGADA", 2015, "mean"),
    list("PROP_POP_EMPREGADA", 2016, "mean"),
    list("PROP_POP_EMPREGADA", 2017, "mean"),
    list("PROP_POP_EMPREGADA", 2018, "mean")

#    ,
    #list("VINCULOS_MEDIA", 2002, "mean"),
    #list("VINCULOS_MEDIA", 2003, "mean"),
    #list("VINCULOS_MEDIA", 2004, "mean"),
    #list("VINCULOS_MEDIA", 2005, "mean"),
    #list("VINCULOS_MEDIA", 2006, "mean"),
#    list("VINCULOS_MEDIA", 2007, "mean"),
#    list("VINCULOS_MEDIA", 2008, "mean"),
#    list("VINCULOS_MEDIA", 2009, "mean"),
#    list("VINCULOS_MEDIA", 2010, "mean"),
#    list("VINCULOS_MEDIA", 2011, "mean"),
#    list("VINCULOS_MEDIA", 2012, "mean"),
#    list("VINCULOS_MEDIA", 2013, "mean"),
#    list("VINCULOS_MEDIA", 2014, "mean"),
#    list("VINCULOS_MEDIA", 2015, "mean"),
#    list("VINCULOS_MEDIA", 2016, "mean"),
#    list("VINCULOS_MEDIA", 2017, "mean"),
#    list("VINCULOS_MEDIA", 2018, "mean")
...
#synth_data new matching
matched_control_group = read.csv(file.path(matching_data_path, 'matching_k_nearest_prop_pop_emprego_rais.csv'))
#matched_control_group = read.csv(file.path(matching_data_path, 'matching_k_nearest_emprego_caged.csv'))
...
ggplot() +
  ...
  scale_y_continuous(
    breaks = seq(0, max(plot_df$Treated, plot_df$Synthetic), by = 0.01),
  ...
  labs(
    ...
    x = "Year",
    y = "Proportion of Employed Population"
  ) +
  theme(
...
sp <- ggplot(gap_data, aes(x = Coefficient, y = Gap, color = Group)) +
  ...
  labs(
    ...
    x = "Year",
    y = "Gap in Proportion of Employed Population"
  ) +
  scale_y_continuous(
    labels = label_number(big.mark = ".", decimal.mark = ",", prefix = " ")
  ) +
...
```{r}
synth.tables <- synth.tab(dataprep.res = synth_data, synth.res = synth_output)
```


```{r}
pred_tab = synth.tables$tab.pred
pred_tab
```
...
for (row in rownames(pred_tab)) {
  #print(pred_tab[row, ])
  row_data = pred_tab[row, ]
  if (grepl("PROP_POP_EMPREGADA", row)) {
    sum_prop = sum_prop + row_data
    count_prop = count_prop + 1
  }
  else {
    cat(row, "&", row_data["Treated"], "&", row_data["Synthetic"], "&", row_data["Sample Mean"], "\n", "\\\\ \\hline", "\n")
  }
}

mean_prop = sum_prop/count_prop
cat("$PropEmployed$", "&", mean_prop[1], "&", mean_prop[2], "&", mean_prop[3], "\n", "\\\\ \\hline", "\n")

```


```{r}
weight_tab = synth.tables$tab.w
weight_tab
```
...
```{r}
for (i in 1:nrow(weight_tab)) {
  
  name = weight_tab[i, "unit.names"]
  code = weight_tab[i, "unit.numbers"]
  weight = weight_tab[i, "w.weights"]
  
  if (weight >= 0.001){
    cat(name, "&", code, "&", weight, "\n", "\\\\ \\hline", "\n")
  }
...
```{r}
placebo_df_filtered_prop_emp = placebo_df_filtered
gap_data_prop_emp = gap_data
rmspe_stats_filtered_prop_emp = rmspe_stats_filtered
treated_ratio_prop_emp = treated_ratio
```



```{r}
placebo_df_filtered = placebo_df_filtered_prop_emp
gap_data = gap_data_prop_emp
rmspe_stats_filtered = rmspe_stats_filtered_prop_emp
treated_ratio = treated_ratio_prop_emp
num_filtered_placebos = length(unique(placebo_df_filtered$NOME_MUN))
...
sp <- ggplot() +
  ...
  annotate("text", x = 2019,
           #y = min(gap_data$Gap),
           y = min(gap_data$Gap, placebo_df_filtered$Gap),
           label = "Treatment Year", hjust = 1.1, vjust = -0.5, size = 3.5) +
  
  # --- Add comment about number of placebos ---
  annotate(
    "text",
    x = min(gap_data$Year),   # leftmost x
    y = min(gap_data$Gap, placebo_df_filtered$Gap),    # topmost y
    label = paste0("Number of placebo units: ", num_filtered_placebos),
    hjust = 0, vjust = -0.5,
    size = 3.5
    ,
    color = "gray30"
...
ggplot(rmspe_stats_filtered, aes(x = ratio)) +
   # --- Add custom gray axes ---
  geom_hline(yintercept = 0, color = "black", linewidth = 0.5) +
  geom_vline(xintercept = 0, color = "black", linewidth = 0.5) +
  
  geom_histogram(
    aes(fill = "Placebo Tests"), 
    color = "black",
    binwidth = 1, alpha = 0.8
  ) +
  geom_vline(
    aes(xintercept = treated_ratio, color = "Treated Unit"),
    size = 1.3
  ) +
  
  annotate("text", x = treated_ratio, y = Inf,
           label = paste0("Treated (p-value = ", round(p_value, 3), ")"),
           hjust = -0.1, vjust = 1,
           #vjust = 2, hjust = 1.1,
           color = "#E87D72", size = 4) +
  scale_x_continuous(
    breaks = seq(0, max(treated_ratio, rmspe_stats_filtered$ratio) + 2, by = 2)
  ) +
  scale_y_continuous(breaks = breaks_width(1)) +
  scale_fill_manual(
    name = "",
    values = c("Placebo Tests" = "#56BCC2")
  ) +
  scale_color_manual(
    name = "",
    values = c("Treated Unit" = "#E87D72")
  ) +
  guides(
...
ggplot(rmspe_stats_filtered, aes(x = ratio)) +
  ...
  annotate("text", x = treated_ratio, y = 0, hjust = -0.1, vjust = -1,
...
#synth_data new matching
matched_control_group = read.csv(file.path(matching_data_path, 'matching_k_nearest_no_downstream_prop_pop_emprego_rais.csv'))
#matched_control_group = read.csv(file.path(matching_data_path, 'matching_k_nearest_emprego_caged.csv'))
