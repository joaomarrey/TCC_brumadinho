import pandas as pd
import os
from paths import final_data_path, matching_data_path, manipulated_data_path
import math
...
# monthly employment
emprego_caged = pd.read_csv(os.path.join(manipulated_data_path, 'empregos_caged_mes_municipios.csv'))
emprego_caged = emprego_caged[['ANO', 'MES', 'COD_MUN', 'VINCULOS_ATIVOS']]

emprego_caged = emprego_caged[emprego_caged['ANO'].isin(pre)]

emprego_caged_pivot = emprego_caged.pivot(index=['COD_MUN'], columns=['ANO', 'MES'])
emprego_caged_pivot.dropna(axis=0, inplace=True, how='any')
print(emprego_caged_pivot)

emprego_caged_pivot_standard = emprego_caged_pivot.apply(zscore, ddof=1)

print(emprego_caged_pivot_standard)
## INCOMPLETE
...
# matching_pivot.to_csv(os.path.join(matching_data_path, 'matching_pivot_emprego_caged.csv'))
# matching_pivot_standard.to_csv(os.path.join(matching_data_path, 'matching_pivot_standard_emprego_caged.csv'))
matched.to_csv(os.path.join(matching_data_path, 'matched_distance_emprego_caged.csv'), index=False)
k_nearest.to_csv(os.path.join(matching_data_path, 'matching_k_nearest_emprego_caged.csv'), index=False)

matched_no_downstream.to_csv(os.path.join(matching_data_path, 'matched_distance_no_downstream_emprego_caged.csv'), index=False)
k_nearest_no_downstream.to_csv(os.path.join(matching_data_path, 'matching_k_nearest_no_downstream_emprego_caged.csv'), index=False)
