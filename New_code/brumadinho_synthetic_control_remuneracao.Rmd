---
title: "brumadinho_synthetic_control_remuneracao"
...
matched_control_group = read.csv(file.path(matching_data_path, 'matching_k_nearest_remuneracao.csv'))

synth_data = dataprep(
  foo = final_df,
  predictors = normal_predictors,
  special.predictors = special_predictors,
  #dependent = "REMUNERACAO_DEZEMBRO",
  dependent = "REMUNERACAO_MEDIA",
...
ggplot() +
  ...
  scale_y_continuous(
    breaks = seq(0, max(plot_df$Treated, plot_df$Synthetic), by = 500),
...
for (row in rownames(pred_tab)) {
  ...
  else if (grepl("REMUNERACAO_MEDIA", row)) {
...
cat("$WAGE$", "&", mean_wage[1], "&", mean_wage[2], "&", mean_wage[3], "\n", "\\\\ \\hline", "\n")
...
```{r}
placebo_df_filtered_wage = placebo_df_filtered
gap_data_wage = gap_data
rmspe_stats_filtered_wage = rmspe_stats_filtered
treated_ratio_wage = treated_ratio
```


```{r}
placebo_df_filtered = placebo_df_filtered_wage
gap_data = gap_data_wage
rmspe_stats_filtered = rmspe_stats_filtered_wage
treated_ratio = treated_ratio_wage
...
#synth_data new matching
matched_control_group = read.csv(file.path(matching_data_path, 'matching_k_nearest_no_downstream_remuneracao.csv'))
