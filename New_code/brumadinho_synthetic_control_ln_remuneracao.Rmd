---
title: "brumadinho_synthetic_control_ln_remuneracao"
...
special_predictors = list(
    ...
    ,
    list("LN_REMUNERACAO_MEDIA", 2002, "mean"),
    list("LN_REMUNERACAO_MEDIA", 2003, "mean"),
    list("LN_REMUNERACAO_MEDIA", 2004, "mean"),
    list("LN_REMUNERACAO_MEDIA", 2005, "mean"),
    list("LN_REMUNERACAO_MEDIA", 2006, "mean"),
    list("LN_REMUNERACAO_MEDIA", 2007, "mean"),
    list("LN_REMUNERACAO_MEDIA", 2008, "mean"),
    list("LN_REMUNERACAO_MEDIA", 2009, "mean"),
    list("LN_REMUNERACAO_MEDIA", 2010, "mean"),
    list("LN_REMUNERACAO_MEDIA", 2011, "mean"),
    list("LN_REMUNERACAO_MEDIA", 2012, "mean"),
    list("LN_REMUNERACAO_MEDIA", 2013, "mean"),
    list("LN_REMUNERACAO_MEDIA", 2014, "mean"),
    list("LN_REMUNERACAO_MEDIA", 2015, "mean"),
    list("LN_REMUNERACAO_MEDIA", 2016, "mean"),
    list("LN_REMUNERACAO_MEDIA", 2017, "mean"),
    list("LN_REMUNERACAO_MEDIA", 2018, "mean")
...
matched_control_group = read.csv(file.path(matching_data_path, 'matching_k_nearest_ln_remuneracao.csv'))

synth_data = dataprep(
  foo = final_df,
  predictors = normal_predictors,
  special.predictors = special_predictors,
  dependent = "LN_REMUNERACAO_MEDIA",
...
ggplot() +
  ...
  scale_y_continuous(
    breaks = seq(0, max(plot_df$Treated, plot_df$Synthetic), by = 0.5),
...
sp <- ggplot(gap_data, aes(x = Coefficient, y = Gap, color = Group)) +
  ...
  theme(text = element_text(size = 10),
        ...
        panel.grid.minor.y = element_line(color = "gray95", size = 0.3),
        legend.position = "bottom") +
  annotate("text", x = "tr2019", y = 1.5*min(gap_data$Gap), label = "Treatment Year", hjust = 1.1)+
...
for (row in rownames(pred_tab)) {
  ...
  if (grepl("PIB_PER_CAPITA", row)) {
    sum_gdp = sum_gdp + row_data
    count_gdp = count_gdp + 1
  }
  else if (grepl("LN_REMUNERACAO_MEDIA", row)) {
    sum_wage = sum_wage + row_data
    count_wage = count_wage + 1
  }
  else {
...
mean_gdp = sum_gdp/count_gdp
cat("$GDPPerCapita$", "&", mean_gdp[1], "&", mean_gdp[2], "&", mean_gdp[3], "\n", "\\\\ \\hline", "\n")

mean_wage = sum_wage/count_wage
cat("$Log WAGE$", "&", mean_wage[1], "&", mean_wage[2], "&", mean_wage[3], "\n", "\\\\ \\hline", "\n")
...
```{r}
placebo_df_filtered_ln_wage = placebo_df_filtered
gap_data_ln_wage = gap_data
rmspe_stats_filtered_ln_wage = rmspe_stats_filtered
treated_ratio_ln_wage = treated_ratio
```
...
```{r}
placebo_df_filtered = placebo_df_filtered_ln_wage
gap_data = gap_data_ln_wage
rmspe_stats_filtered = rmspe_stats_filtered_ln_wage
treated_ratio = treated_ratio_ln_wage
...
#synth_data new matching
matched_control_group = read.csv(file.path(matching_data_path, 'matching_k_nearest_no_downstream_ln_remuneracao.csv'))
