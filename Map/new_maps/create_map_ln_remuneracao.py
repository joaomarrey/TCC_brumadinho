...
# with downstream
matched = pd.read_csv(os.path.join(matching_data_path, 'matching_k_nearest_ln_remuneracao.csv'))
...
# ax.set_title("Log Average Wage Synthetic Control Municipalities With Downstream (Minas Gerais State)")
plt.axis('off')
...
# plt.show()
plt.tight_layout()
plt.savefig('Mapas_matching/ln_remuneracao_municipios_map.png', dpi=500, bbox_inches='tight')
# plt.savefig("ln_remuneracao_vectorized.pdf")

#merged.to_file('SHPS_matching/ln_remuneracao_municipios_shape.shp')


# no downstream
matched = pd.read_csv(os.path.join(matching_data_path, 'matching_k_nearest_no_downstream_ln_remuneracao.csv'))
merged = gpd.read_file(os.path.join(maps_data, 'MG_Municipios_2023.shp'))
...
# ax.set_title("Log Average Wage Synthetic Control Municipalities Without Downstream (Minas Gerais State)")
plt.axis('off')
ax.legend(handles=legend_elements, loc='lower left',
          # title='Treatment and Downstream Municipalities',
          #title='Legend'
...
# plt.show()
plt.tight_layout()
plt.savefig('Mapas_matching/ln_remuneracao_municipios_map_no_dowsntream.png', dpi=500, bbox_inches='tight')
# plt.savefig("ln_remuneracao_vectorized.pdf")

#merged.to_file('SHPS_matching/ln_remuneracao_municipios_shapeno_dowsntream.shp')
